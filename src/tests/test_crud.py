...
class TestGetObject:
    def test_get_object(self, session, admin_user):
        ...
        object_in_db = base_services.get_object(session=session, statement=statement)
    ...
    def test_get_not_exists_object(self, session):
        with pytest.raises(HTTPException) as exception_info:
            ...
            base_services.get_object(session=session, statement=statement)
...
class TestDeleteObject:
    def test_exists_object(self, session, admin_user):
        ...
        assert not base_services.is_object_exists(session=session, statement=User.id == admin_user.get("id"))
